...
--
-- INSERE CALENDÁRIO MPOG 2019
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-01-01','confraternização universal (feriado nacional)');
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-03-04','carnaval (ponto facultativo)');
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-03-05','carnaval (ponto facultativo)');
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-03-06','quarta-feira de cinzas (ponto facultativo até as 14 horas)');
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-04-19','paixão de cristo (feriado nacional)');
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-04-21','tiradentes (feriado nacional)');
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-05-01','dia mundial do trabalho (feriado nacional)');
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-05-31','corpus christi (ponto facultativo)');
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-09-07','independência do brasil (feriado nacional)');
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-10-12','nossa senhora aparecida (feriado nacional)');
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-10-28','dia do servidor público - art. 236 da lei nº 8.112, de 11 de dezembro de 1990 (ponto facultativo)');
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-11-02','finados (feriado nacional)');
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-11-15','proclamação da república (feriado nacional) e');
insert into fks_feriados(dt_feriado,name_feriado) values ('2019-12-25','natal (feriado nacional)');
...
BEGIN
    ...
    RETURN CONCAT(horaCalc,':',minutoCalc,':',segundoCalc);
END$$

DELIMITER ;

DROP function IF EXISTS fncMinToTime2;

DELIMITER $$
CREATE FUNCTION fncMinToTime2 (minTime integer)
RETURNS VARCHAR(10)
BEGIN
  DECLARE iHora INT;
    DECLARE iMinuto INT;
    DECLARE iSegundo INT;
    
  SET iHora = FLOOR(minTime / 60);
  SET iMinuto = TRUNCATE(MOD(minTime, 60),0);
  SET iSegundo = TRUNCATE((MOD(minTime, 60) - TRUNCATE(MOD(minTime, 60),0)) * 60,0);
  return CONCAT(lpad(iHora,2,'0'),':',lpad(iMinuto,2,'0'),':',lpad(iSegundo,2,'0'));
END$$
      ...
      WHILE (iCount <= countDias) DO
          SET dataProxima = DATE_ADD(dtIni, INTERVAL iCount DAY);
            ...
            ELSE
                ...
                -- Verificar Feriado
                  IF ((SELECT COUNT(*) FROM fks_feriados WHERE dt_feriado = DATE(dataProxima)) <= 0) THEN
                        SET dataCalcIni = CAST(CONCAT(YEAR(dataProxima),'-',MONTH(dataProxima),'-',DAY(dataProxima),' ', iniTurno) AS DATETIME);
...
select
    1 as id_week,
    'domingo' as day_of_week
union all select
    2 as id_week,
    'segunda' as day_of_week
union all select
    3 as id_week,
    'terça' as day_of_week
union all select
    4 as id_week,
    'quarta' as day_of_week
union all select
    5 as id_week,
    'quinta' as day_of_week
union all select
    6 as id_week,
    'sexta' as day_of_week
union all select
    7 as id_week,
    'sábado' as day_of_week
;
drop view if exists vw_hour_by_hour;
create view vw_hour_by_hour as 
select
    1 as id_time,
    '00:00:00' as time_ini,
    '07:59:59' as time_fin
union all select
    2 as id_time,
    '08:00:00' as time_ini,
    '08:59:59' as time_fin
union all select
    3 as id_time,
    '09:00:00' as time_ini,
    '09:59:59' as time_fin
union all select
    4 as id_time,
    '10:00:00' as time_ini,
    '10:59:59' as time_fin
union all select
    5 as id_time,
    '11:00:00' as time_ini,
    '11:59:59' as time_fin
union all select
    6 as id_time,
    '12:00:00' as time_ini,
    '12:59:59' as time_fin
union all select
    7 as id_time,
    '13:00:00' as time_ini,
    '13:59:59' as time_fin
union all select
    8 as id_time,
    '14:00:00' as time_ini,
    '14:59:59' as time_fin
union all select
    9 as id_time,
    '15:00:00' as time_ini,
    '15:59:59' as time_fin
union all select
    10 as id_time,
    '16:00:00' as time_ini,
    '16:59:59' as time_fin
union all select
    11 as id_time,
    '17:00:00' as time_ini,
    '17:59:59' as time_fin
union all select
    12 as id_time,
    '18:00:00' as time_ini,
    '18:59:59' as time_fin
union all select
    13 as id_time,
    '19:00:00' as time_ini,
    '19:59:59' as time_fin
union all select
    14 as id_time,
    '20:00:00' as time_ini,
    '23:59:59' as time_fin
;
drop view if exists vw_last_7_day;
create view vw_last_7_day as 
select
    (curdate() - interval 7 day) as last7day
union all select
    (curdate() - interval 6 day) as last7day
union all select
    (curdate() - interval 5 day) as last7day
union all select
    (curdate() - interval 4 day) as last7day
union all select
    (curdate() - interval 3 day) as last7day
union all select
    (curdate() - interval 2 day) as last7day
union all select
    (curdate() - interval 1 day) as last7day
union all select
    curdate() as last7day
;
drop view if exists vw_last_7_day_by_hour;
create view vw_last_7_day_by_hour as 
select
    vw_last_7_day.last7day as last7day,
    vw_hour_by_hour.id_time as id_time,
    vw_hour_by_hour.time_ini as time_ini,
    vw_hour_by_hour.time_fin as time_fin
from
    (vw_last_7_day
join vw_hour_by_hour)
order by
    vw_last_7_day.last7day
;
drop view if exists vw_week_day_by_hour;
create view vw_week_day_by_hour as 
select
    vw_hour_by_hour.id_time as id_time,
    vw_hour_by_hour.time_ini as time_ini,
    vw_hour_by_hour.time_fin as time_fin,
    vw_day_of_week.id_week as id_week,
    vw_day_of_week.day_of_week as day_of_week
from
    (vw_hour_by_hour
join vw_day_of_week)
order by
    vw_day_of_week.id_week,
    vw_hour_by_hour.id_time
;
drop view if exists vw_total_week_last_7_day;
create view vw_total_week_last_7_day as 
select
    date_format(dh.last7day, '%U') AS weekOfYear,
    count(t.id) AS qtd
from
    (vw_last_7_day_by_hour dh
left join ticket t on
    (((cast(t.create_time as date) = dh.last7day)
    and (cast(t.create_time as time) between dh.time_ini and dh.time_fin))))
where
    (1 = 1)
group by
    date_format(dh.last7day, '%U')
order by
    dh.last7day,
    dh.id_time
;
...
group by th.ticket_id
;
drop view if exists vw_first_history_new;
create view vw_first_history_new as 
select
...
group by
    th.ticket_id
;
drop view if exists vw_first_lock;
create view vw_first_lock as 
select
    min(th.id) AS history_id,
    th.ticket_id AS ticket_id
from
...
group by ticket_id
;
drop view if exists vw_has_send_mail;
create view vw_has_send_mail as 
select
    min(th.id) AS history_id,
    th.ticket_id AS ticket_id
from
...
where 1=1
;
drop view if exists vw_ticket_by_service;
create view vw_ticket_by_service as 
select
    s.id AS service_id,
    s.name AS service_name,
    count(t.id) AS qtd_ticket
from
    (ticket t
join service s on
    ((t.service_id = s.id)))
group by
    ...
    s.name
;
drop view if exists vw_ticket_dynamic_field;
create view vw_ticket_dynamic_field as 
select
    ticket_history.id AS history_id,
    ticket_history.ticket_id AS ticket_id,
    replace(substring_index(ticket_history.name, '%%', 3),
    '%%FieldName%%',
    '') AS field_name,
    substr(ticket_history.name,
    (locate('Value%%', ticket_history.name) + 7),
    (locate('%%OldValue', ticket_history.name) - (locate('Value%%', ticket_history.name) + 7))) AS field_value,
    (case
        when (substr(ticket_history.name,
        (locate('Value%%', ticket_history.name) + 7),
        (locate('%%OldValue', ticket_history.name) - (locate('Value%%', ticket_history.name) + 7))) = 0) then 'Não identificado'
        when (substr(ticket_history.name,
        (locate('Value%%', ticket_history.name) + 7),
        (locate('%%OldValue', ticket_history.name) - (locate('Value%%', ticket_history.name) + 7))) = 1) then 'Usuário'
        when (substr(ticket_history.name,
        (locate('Value%%', ticket_history.name) + 7),
        (locate('%%OldValue', ticket_history.name) - (locate('Value%%', ticket_history.name) + 7))) = 2) then 'Software'
        when (substr(ticket_history.name,
        (locate('Value%%', ticket_history.name) + 7),
        (locate('%%OldValue', ticket_history.name) - (locate('Value%%', ticket_history.name) + 7))) = 3) then 'Hardware'
        else substr(ticket_history.name,
        (locate('Value%%', ticket_history.name) + 7),
        (locate('%%OldValue', ticket_history.name) - (locate('Value%%', ticket_history.name) + 7)))
    end) AS field_value_name,
    substr(ticket_history.name,
    (locate('OldValue%%', ticket_history.name) + 10),
    length(substr(ticket_history.name,
    (locate('OldValue%%', ticket_history.name) + 10)))) AS field_old_value
from
    ticket_history
where
    (ticket_history.history_type_id = 28)
;
drop view if exists vw_tickets_inconsistentes;
create view vw_tickets_inconsistentes as 
select
    distinct th.ticket_id AS ticket_id
from
    (ticket_history th
join ticket t on
    ((th.ticket_id = t.id)))
...
order by
    th.ticket_id
;
drop view if exists vw_tickets_emails;
create view vw_tickets_emails as 
select
    hsm.ticket_id AS ticket_id,
    th.create_time AS time_send_mail
from
    (vw_has_send_mail hsm
join ticket_history th on
    ((hsm.history_id = th.id)))
;
drop view if exists vw_tickets_1_tratamento;
create view vw_tickets_1_tratamento as 
select
    tm.history_id AS history_id,
    tm.ticket_id AS ticket_id,
    th.queue_id AS queue_id,
    th.state_id AS state_id,
    (case
        when (th.queue_id = 14) then hl.change_time
        else th.create_time
    end) AS time_first_move
from
    (((vw_first_move tm
join ticket_history th on
    ((tm.history_id = th.id)))
left join vw_last_closed lc on
    ((tm.ticket_id = lc.ticket_id)))
left join ticket_history hl on
    ((lc.history_id = hl.id)))
;
...
te.user_finish,
sv.id as vote_id, fncRemTagHtml(replace(sv.vote_value,'$html/text$','')) as vote_value
, sa.answer
...
select 
  ...
  t.sla_id as sla_id,
  sla.name as sla_name,
  -- replace(substring(sla.comments,1,instr(sla.comments,';')-1),'ust=') as sla_ust, 
  -- replace(substring(sla.comments,instr(sla.comments,';')+1,length(sla.comments)),'vlr=') as sla_ust_vlr,
